...
var Hit = React.createClass({
    ...
    render: function render() {
        ...
        return React.createElement(
            "span",
            { className: spanClass, style: style },
...
var Round = React.createClass({
    ...
    render: function render() {
        ...
        var hits = this.props.round.Hits.map(function (hit) {
            return React.createElement(Hit, { key: hit.ID, hit: hit, round: round, roundDuration: duration, firstFighterId: firstFighterId });
        });
...
var Rounds = React.createClass({
    ...
    render: function render() {
        ...
        for (var i = rounds.length - 1; i >= 0; i--) {
            var round = rounds[i];
            result.push(React.createElement(Round, { round: round, key: round.ID, firstFighterId: firstFighterId, roundNumber: roundNumber-- }));
        }
